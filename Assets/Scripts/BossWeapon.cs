...
{
    [SerializeField]
    private GameObject projectilePrefab; //공격할 떄 생성되는 발사체 프리팹 
    ...
    private IEnumerator CircleFire()
    {
        ...
        float intervalAngel = 360/count; //발사체 사이의 각도
        float weightAngle = 0; //가중되는 각도(항상 같은 위치로 발사되지 않도록 설정) 
        ...
        while (true)
        {
            for (int i = 0; i<count; i++)
            {
                ...
                //발사체 이동 방향(각도) 
                float angle = weightAngle + intervalAngel * i;
                //발사체 이동 방향(백터) 
